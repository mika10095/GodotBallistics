...
func _physics_process(delta):
	if player.input_dir != Vector2.ZERO:
		...
		pivot.position.z = lerp(pivot.position.z, bob_vector.x * bob_dist, delta * lerp_speed)
		pivot.position.x = lerp(pivot.position.x, bob_vector.x * bob_dist, delta * lerp_speed)
	else:
